...
### Changed
- **interview/research**: Replace blanket per-question six-lane advisory fan-out with one bounded start-turn factual snapshot (`code_context` + `web_context`) published through the project artifact store. Complete-cache follow-ups emit zero advisory subagents and fetch lane-scoped evidence; partial, stale, malformed, or missing baselines rerun only the missing factual lane. Measurements run directly only when a question needs them, while milestone lateral review and the seed-ready acceptance guard remain fresh independent checks.
