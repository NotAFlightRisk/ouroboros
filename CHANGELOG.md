...
### Changed
- **gjc/integration**: Replace the GJC input-interception bridge with an OMP-style runtime projection. `ouroboros setup --runtime gjc` now installs namespaced `ouroboros-*` skills into GJC's native user skill registry, registers the isolated Ouroboros MCP server through `gjc mcp`, preserves user-managed MCP entries, and removes the obsolete setup-owned bridge.
