...
### Changed
- **gjc/integration**: Replace the GJC input-interception bridge with an OMP-style runtime projection when the installed GJC proves conventional standalone MCP autoload. `ouroboros setup --runtime gjc` installs namespaced `ouroboros-*` skills, registers the isolated Ouroboros MCP server through `gjc mcp`, preserves user-managed MCP entries, and removes the obsolete setup-owned bridge only after activation succeeds. Storage-only releases such as GJC 0.12.7 now fail before projection/config migration and preserve any legacy route instead of claiming native runtime activation.
