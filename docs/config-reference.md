...
| `OUROBOROS_KNOWN_DATA_TOOLS` | _(data-lane hint, proposal-only)_ | Comma-separated MCP tool names the interview `data_context` advisory lane is told about as **proposal-only** hints (e.g. `clickhouse_query,metabase_card`): the child never executes them directly — it returns `proposed_queries` against them, which the parent session runs only after explicit user confirmation. A name alone cannot prove read-onlyness, so no name earns direct execution from this list. Each entry must match `[A-Za-z][A-Za-z0-9_.-]{0,63}` (no colon — it reads as a credential assignment at re-entry); invalid entries AND identifiers carrying a mutating verb (e.g. `delete_database`) are dropped, and at most 16 are used. Trusted-source only: it steers which tools the data child is told to call, so an untrusted project `.env` cannot set it — only the real environment or `~/.ouroboros/.env`. |
| `OUROBOROS_KNOWN_DATA_TOOLS_READONLY` | _(data-lane hint, direct execution)_ | The operator's **explicit declaration** that the listed tools are read-only — the only way a configured tool name reaches the lane's `known_data_tools` (direct-execution steering for local, free, read-only lookups). Same grammar, filtering, and trusted-source rules as `OUROBOROS_KNOWN_DATA_TOOLS`. Declare a tool here only when you know it cannot mutate state; when unsure, leave it in the plain variable and confirm its proposed queries per use. |
