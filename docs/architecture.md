...
- Failures are handled by an attempt-then-bounce loop rather than ever-deeper pre-execution splitting. Each atomic provider attempt targets at most `execution.max_iterations_per_ac` tool-bearing agent turns (default 10): the common stream owner cancels at the first observed over-budget turn and admits no later turn. `execution.ac_attempt_timeout_seconds` is a fixed total wall-clock bound (default 900). Activity resets the separate idle watchdog but never either boundary; exhaustion goes directly to bounce classification or failure instead of same-AC retry. Legacy whole-Seed direct/resume calls deterministically scale both per-AC limits by the number of root ACs and terminally fail without recovery or route escalation when exhausted.
