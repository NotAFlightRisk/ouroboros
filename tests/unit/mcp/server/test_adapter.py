...
""".strip()
    ...
    def test_seed_coverage_survives_partial_production_extraction(self, tmp_path: Any) -> None:
        ...
        seed = SimpleNamespace(
            ...
            acceptance_criteria=("MUST create file marker.txt", "MUST add file docs.md"),
        )
        mechanical = _parse_legacy_execution_task_summary(
    ...
    def test_stale_result_identity_cannot_hide_behind_a_trusted_report_label(self) -> None:
        ...
        stale_result = SpecVerificationResult(
            assertion=stale_assertion,
            verified=True,
            detail="Found structure 'Unrelated'",
        )
        stale_report = ACVerificationReport.model_construct(
            ac_index=0,
            ac_text=trusted_text,
            results=(stale_result,),
            agent_reported_pass=True,
        )
        verification = SimpleNamespace(reports=(stale_report,))
        mechanical = EvaluationSummary(
            final_approved=True,
            highest_stage_passed=2,
            task_results=(
        ...
        assert summary.final_approved is False
        assert summary.ac_results[0].passed is False
        assert "assertion text" in summary.ac_results[0].evidence
        assert "parent report" in summary.ac_results[0].evidence
