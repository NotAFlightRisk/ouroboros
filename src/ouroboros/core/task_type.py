...
_POST_MATCH_REJECTION_PATTERN = re.compile(
    ...
    r"|scratch\s+that(?=\s*$)"
    r"|(?:should|must|may)\s+not\s+(?:be\s+)?(?:used|required|adopted|applied|selected|chosen)"
    r"|(?:should|must|may)\s+(?:be\s+)?avoided"
    r"|(?:was|is|has\s+been)\s+ruled\s+out"
    r"|(?:should|must|may)\s+not\s+(?:be\s+)?(?:used|required|adopted|applied|selected|chosen)"
    r"|(?:value|requirement|contract|proposal)\s+(?:should|must|may)\s+not\s+(?:be\s+)?(?:used|required|adopted|applied|avoided|selected|chosen)"
