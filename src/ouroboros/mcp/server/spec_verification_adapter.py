...
def _report_identity_error(
    ...
    trusted_content = _trusted_ac_content_sources(mechanical, seed_criteria)
    ...
    for position, report in enumerate(reports):
        ...
        if seed_criteria:
            ...
            if expected_text is not None and report_text != expected_text:
                mismatches = [
                    (source, content)
                    for source, content in trusted_content.get(ac_index, ())
                    if report_text != content
                ]
                rendered = "; ".join(
                    f"{source} criterion {content!r}" for source, content in mismatches
                )
                return (
                    f"report AC {ac_index + 1} text does not match the authoritative Seed AC; "
                    f"provenance mismatch: report criterion {report_text!r} does not match "
                    f"{rendered}"
                )

        trusted_sources = trusted_content.get(ac_index, ())
        if seed_criteria:
            trusted_sources = tuple(
                (source, content) for source, content in trusted_sources if source == "seed"
            )
        if not trusted_sources:
            return f"report AC {ac_index + 1} has no trusted acceptance-criterion input"
        mismatches = [
        ...
        if mismatches:
            ...
            return (
                f"report AC {ac_index + 1} provenance mismatch: report criterion "
                f"{report_text!r} does not match {rendered}"
            )
...
def evaluation_summary_from_spec_verification(
    ...
    identity_error = _report_identity_error(reports, seed_criteria, mechanical)
