    ...
    def definition(self) -> MCPToolDefinition:
        ...
        return MCPToolDefinition(
            ...
            description=(
                ...
                "/ `unknown_fanout_id` are clean errors. CONSENT: a data_context "
                "result loses its human-facing narrative once it is retained "
                "server-side, so any outcome carrying "
                "`consent_status: not_confirmable_prose_not_retained` (a replay) is NOT "
                "ready to forward as a data-derived answer — re-run the "
                "advisory fan-out to obtain a confirmable one. A data result "
                "sent in an earlier finalize=false call is not carried "
                "forward either: the accumulating response lists it under "
                "`not_retained_keys` instead of `received_keys`, and the "
                "closing call reports it under `missing_optional_keys` "
                "until resent, so submit the data lane in the call that "
                "finalizes. "
                "Submissions are "
