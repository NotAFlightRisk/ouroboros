...
class AutoStore:
    ...
    def save(self, state: AutoPipelineState) -> Path:
        ...
        Auto pipeline state carries the interview answers the user confirmed,
        including any marked ``[from-data]``, and it lives for as long as the
        session record does. It is the same artifact class the interview
        transcript and the Seed belong to, so it takes the same primitive
        (round-68) — writing it through ``write_text`` left it at the umask
        default, typically ``0644``, for its whole lifetime.
        ...
        not own is not its to do (round-60).
        """
        state._validate_loaded()
