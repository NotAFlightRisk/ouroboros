...
_MAX_PARSE_DEPTH = 40
_MAX_UNANCHORED_REPEAT_WIDTH = 128
...
def _branch_count(sequence: object, depth: int = 0) -> int:
    """Count backtrackable branch regions, capped once the proof must refuse."""
    if depth > _MAX_PARSE_DEPTH:
        return 2
    try:
        items = list(sequence)  # type: ignore[call-overload]
    ...
    for opcode, argument in items:
        ...
        if name == "BRANCH":
            ...
            _reference, yes_arm, no_arm = argument
            children = (yes_arm,) if no_arm is None else (yes_arm, no_arm)
        for child in children:
            total += _branch_count(child, depth + 1)
            if total > 1:
                return 2
    return total
...
def _repeat_atoms(
    ...
    """Return every atom in a positive fixed-width simple repeat body."""
    if depth > _MAX_PARSE_DEPTH:
        return None
    try:
        items = list(sequence)  # type: ignore[call-overload]
    ...
    for opcode, argument in items:
        ...
        if name in {"LITERAL", "NOT_LITERAL", "IN", "ANY", "CATEGORY"}:
            atoms.append((name, argument))
        elif name == "GROUPREF" and argument in fixed_groups:
            return None
        elif name == "SUBPATTERN":
            nested = _repeat_atoms(argument[-1], fixed_groups, depth + 1)
            if nested is None:
                return None
            atoms.extend(nested)
        else:
...
def _category_matches_literal(category: object, character: str) -> bool:
    ...
    if name.endswith("_SPACE"):
        matched = character.isspace()
    elif name.endswith("_DIGIT"):
        matched = character.isdecimal()
    elif name.endswith("_WORD"):
        matched = character == "_" or character.isalnum()
...
def _has_unanchored_repeat_retry(
    sequence: object,
    fixed_groups: frozenset[int],
    depth: int = 0,
    terminal_context: bool = True,
) -> bool:
    """Reject repeats that can rescan an unbounded suffix from every byte.

    A terminal repeat cannot fail on a following clause, so successive matches
    consume disjoint runs. A nonterminal large repeat needs a literal prefix
    containing at least one character its body cannot consume; that character
    prevents candidate starts from occurring inside the suffix already scanned
    by an earlier attempt. Unknown and nested shapes fail closed.
    """
    if depth > _MAX_PARSE_DEPTH:
        return True
    try:
    ...
    prefix_literals: list[tuple[str, object]] = []
    for index, (opcode, argument) in enumerate(items):
        item_is_terminal = terminal_context and index == len(items) - 1
        name = getattr(opcode, "name", str(opcode))
        if name == "LITERAL":
            prefix_literals.append((name, argument))
            continue
        if name == "SUBPATTERN":
            body = argument[-1]
            if _has_unanchored_repeat_retry(body, fixed_groups, depth + 1, item_is_terminal):
                return True
            try:
                subitems = list(body)
            except TypeError:
                return True
            if all(getattr(op, "name", str(op)) == "LITERAL" for op, _arg in subitems):
                prefix_literals.extend(("LITERAL", arg) for _op, arg in subitems)
            continue
        if name == "ATOMIC_GROUP":
            if _has_unanchored_repeat_retry(argument, fixed_groups, depth + 1, item_is_terminal):
                return True
            continue
        if name in {"ASSERT", "ASSERT_NOT"}:
            if _has_unanchored_repeat_retry(argument[1], fixed_groups, depth + 1, False):
                return True
            continue
        if name == "BRANCH":
            if any(
                _has_unanchored_repeat_retry(branch, fixed_groups, depth + 1, item_is_terminal)
                for branch in argument[1]
            ):
                return True
            continue
        if name == "GROUPREF_EXISTS":
            _reference, yes_arm, no_arm = argument
            if _has_unanchored_repeat_retry(yes_arm, fixed_groups, depth + 1, item_is_terminal) or (
                no_arm is not None
                and _has_unanchored_repeat_retry(no_arm, fixed_groups, depth + 1, item_is_terminal)
            ):
                return True
            continue
        if name not in _REPEATS:
        ...
        minimum, maximum, body = argument
        if _has_unanchored_repeat_retry(body, fixed_groups, depth + 1, False):
            return True
        if item_is_terminal and minimum <= 1:
            continue
        try:
            _body_minimum, body_maximum = body.getwidth()
        except AttributeError:
            return True
        if (
            maximum != regex_parser.MAXREPEAT
            and body_maximum < regex_parser.MAXWIDTH
            and maximum * body_maximum <= _MAX_UNANCHORED_REPEAT_WIDTH
        ):
            continue
        atoms = _repeat_atoms(body, fixed_groups, depth + 1)
        if atoms is None or not any(
            all(_atoms_are_disjoint(prefix, atom) for atom in atoms) for prefix in prefix_literals
        ):
            return True
    return False
...
def pattern_has_bounded_execution(
    ...
    *,
    ...
    if _contains_scoped_flags(parsed) and _contains_repeat(parsed):
        return False
    if _branch_count(parsed) > 1:
        return False
    _minimum, maximum = parsed.getwidth()
    effective_flags = parsed.state.flags
    if effective_flags & re.ASCII and _contains_repeat(parsed):
        return False
    start_anchored = externally_anchored or _is_start_anchored(parsed, effective_flags)
    if maximum > 0 and not start_anchored and _contains_unbounded_lazy_repeat(parsed):
        return False
    allow_repeated_assertions = start_anchored
    fixed_groups = frozenset(groups)
    return not _has_ambiguous_repeats(
        parsed,
        fixed_groups,
        allow_repeated_assertions,
    ) and (start_anchored or not _has_unanchored_repeat_retry(parsed, fixed_groups))
